...
|---|---|---|---|---|
| **목적지 이동형**: 다른(이미 존재하는) 화면으로 이동 | **"OO로"** (목적지 명사 + "로") | `ArrowLeft` (`h-4 w-4`) | `<Link href={...}>` | 상세 화면 상단 복귀 — `returnTo`가 목록이면 **"목록으로"**, 추천위저드 결과 화면이면 **"추천 결과로"**(`isRecommendReturnTo`, §작업 근거: `ActivityDetail.tsx`/`PlaceDetail.tsx`) |
| **상태 리셋형**: 화면 이동 없이 현재 폼/위저드를 초기 상태로 되돌림 | **"처음부터"** | `ArrowLeft` (`h-4 w-4`) | `onClick={reset}` (로컬 함수, URL도 초기값으로 함께 되돌림) | 추천위저드 결과 화면 + 마법사(중간 단계) 화면 상단 버튼 — 위치는 같아도 동작은 항상 `reset()`이므로 라벨도 항상 "처음부터"로 통일(`ActivityRecommendWizard.tsx`/`PlaceRecommendWizard.tsx`) |
...
**규칙:**
- 같은 좌상단 위치·같은 `ArrowLeft` 아이콘이라도 **동작이 다르면 라벨도 달라야 한다.** 목적지 이동형과 상태 리셋형을 같은 라벨("홈으로" 등)로 뭉치지 않는다 — 과거 위저드 마법사 화면 상단 버튼이 `<Link href="/">홈으로</Link>`였던 것을 "처음부터"(`reset()`)로 교체한 것이 이 판단 기준의 적용 사례다.
