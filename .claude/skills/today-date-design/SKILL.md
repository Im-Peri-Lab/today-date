...
- **"오늘" 단축**(`dpToday`): **헤더 우측**, 다음 달 화살표 바로 옆(`dpHeaderRight` 클러스터 = 다음 화살표 + 오늘). 표준 패턴(노션·구글 캘린더). 작은 텍스트 버튼 — 화살표(`dpNav`)와 같은 높이 2rem·radius 8px, font 13px. **평상시 배경 없음(투명) + `--s-sub` 글씨**, hover/active 시에만 화살표와 동일 surface(`--s-card-border-strong`+`--s-ink`). 평상시 surface를 주지 않는 이유: 화살표 hover 톤과 겹쳐 "이미 눌린 듯" 보이는 잔상 충돌을 막기 위함(평상시 토큰 ≠ 다른 컨트롤 hover 토큰). 클릭 시 **표시 월만** 오늘로 이동(선택값 불변). 그리드 아래엔 버튼을 두지 않는다(팝업 하단 padding = 다른 모서리와 동일, 빈 공간 없음).
