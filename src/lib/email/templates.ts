 ...
 * 파트너 초대 메일.
 *
 * 인증/재설정 메일과 달리 "이미 우리 앱을 쓰는 사람이 보낸 초대"라는 맥락이 있으므로
 * 초대한 사람의 이메일을 함께 보여준다 — 모르는 사람의 초대를 눌러 합류하는 일이
 * 없어야 한다. 만료는 24시간(§ INVITE_EXPIRY_MINUTES).
 */
export function getInvitePartnerTemplate(inviteUrl: string, inviterEmail: string): string {
  return `
<!DOCTYPE html>
<html lang="ko">
<head>
    ...
    <tr>
      <td align="center">
        <table width="480" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(109,40,217,0.08);">
          ...
          <tr>
            <td style="padding:40px;">
              <h2 style="margin:0 0 12px;color:#1f1f1f;font-size:20px;font-weight:700;">파트너 초대가 도착했어요</h2>
              <p style="margin:0 0 24px;color:#6b7280;font-size:15px;line-height:1.6;">
                <strong>${inviterEmail}</strong> 님이<br>
                둘만의 데이트 위시리스트에 초대했습니다.
              </p>
              <table cellpadding="0" cellspacing="0" style="margin:0 auto;">
                <tr>
                  <td style="border-radius:10px;background:#6d28d9;">
                    <a href="${inviteUrl}" style="display:inline-block;padding:14px 36px;color:#ffffff;text-decoration:none;font-size:15px;font-weight:600;border-radius:10px;">
                      초대 수락하기
                    </a>
                  </td>
                </tr>
