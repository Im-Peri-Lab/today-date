...
- 열기 방식은 링크 성격으로 분기(`anchorProps`): **웹 URL(http/https)은 새 탭**(`target="_blank"` + `rel="noopener noreferrer"` — 참고 링크와 동일 패턴), **앱 스킴(`tmap://` 등, `requiresApp: true`)은 같은 탭**(OS가 가로채 앱 실행 → 빈 탭 방지). 단, iOS Safari의 `requiresApp` 앱은 `buildIosSafari`에 등록된 공식 HTTPS 브리지로 같은 탭 이동한다.
...
- **`requiresApp` 앱 미설치 UX — 브라우저별 한계(감지는 best-effort, `MapLink`)**: 커스텀 스킴(`tmap://`)은 미설치 여부를 표준적으로 감지할 방법이 없어 브라우저마다 동작이 갈린다.
  ...
  - **iOS Safari**(`useIsIosSafari`): 미설치 커스텀 스킴의 네이티브 오류와 설치 오탐을 피하기 위해 **`buildIosSafari`의 공식 HTTPS 브리지로 즉시 이동**한다. TMAP 브리지가 iOS 전용 검색 스킴(`tmap://?search=...`)으로 앱 실행을 시도하고, 앱이 없으면 공식 다운로드 안내를 제공한다. 사전 설치 토스트와 추가 [열기] 클릭은 사용하지 않는다.
...
- **추천 위저드**(`ActivityRecommendWizard.tsx`): Step 3(시간대 다음, 카테고리 앞)에 독립 단계로 배치 — 카테고리("무엇을 하는가")와 실내/실외("조건")는 서로 다른 축이라 한 화면에 섞지 않는다(카테고리 개편 때 확립한 "축 분리" 원칙과 일관, §10-J 폼 순서와는 별개로 위저드는 자체 단계 순서를 가짐: 소요시간→시간대→실내/실외→카테고리). UI는 `optionCard` 카드 2개(Step1/2와 동일하게 **선택 시 자동 다음 단계 진행**) + "다음" 버튼(`detailPrimaryBtn` 보라색 CTA, 명시적 스킵 — 장소 위저드의 지역(area) 스킵 버튼과 동일 스타일·문구로 통일, `LOCATION_TYPE_ICONS`/`LOCATION_TYPE_OPTIONS` 재사용). `duration==='overnight'` 시 시간대 단계를 건너뛰는 기존 로직은 유지되며 건너뛴 뒤 도착지가 실내/실외 단계(Step 3)로 바뀐다. **필터는 area와 동일하게 선택적**(`recommendActivities`의 `location_type` truthy 체크 — 미선택 시 필터 미적용) — 저장값(NOT NULL)과 무관한 별개 관심사라는 원칙은 `/list` 필터와 동일.
- **카드 배지**: `ActivityCard`의 기존 메타 줄(소요시간·시간대 표시 줄)에 인라인 항목으로 추가, **줄 맨 앞**에 배치(`소요시간` 앞). 필수 필드라 조건부 렌더 없이 항상 표시, 아이콘 크기는 카드 메타 줄 관례대로 `h-3 w-3`(상세의 `h-3.5`보다 한 단계 작음).
...
왜: "뒤로 가는 듯한" 버튼이 화면마다 "목록으로"·"홈으로"·"처음부터"·"취소" 등으로 흩어져 있으면, 같은 위치(좌상단 `ArrowLeft` + 텍스트)에 있어도 실제로는 서로 다른 세 가지 동작(다른 화면으로 이동 / 현재 화면을 초기 상태로 리셋 / 오버레이 닫기)이 섞여 사용자가 결과를 예측하기 어렵다. 버튼의 **동작 종류**로 라벨 패턴을 고정한다.

**판단 기준 — "이 버튼을 누르면 실제로 무슨 일이 일어나는가?"**

| 동작 종류 | 라벨 패턴 | 아이콘 | 트리거 | 적용처 |
|---|---|---|---|---|
| **목적지 이동형**: 다른(이미 존재하는) 화면으로 이동 | **"OO로"** (목적지 명사 + "로") | `ArrowLeft` (`h-4 w-4`) | `<Link href={...}>` | 상세 화면 상단 복귀 — `returnTo`가 목록이면 **"목록으로"**, 추천위저드 결과 화면이면 **"추천 결과로"**(`isRecommendReturnTo`, §작업 근거: `ActivityDetail.tsx`/`PlaceDetail.tsx`). 추천위저드 마법사 **1단계**(진입 직후, 아직 아무 선택도 하지 않은 상태) 상단 버튼도 **"홈으로"**+`<Link href="/">` — 되돌릴 선택값이 없어 리셋이 무의미하고, 실제 유일한 동작이 홈 이탈 자체이기 때문 |
| **상태 리셋형**: 화면 이동 없이 현재 폼/위저드를 초기 상태로 되돌림 | **"처음부터"** | `ArrowLeft` (`h-4 w-4`) | `onClick={reset}` (로컬 함수, URL도 초기값으로 함께 되돌림) | 추천위저드 결과 화면 상단·하단 + 마법사 **2단계 이상**(이미 선택값이 있어 되돌릴 대상이 존재하는 단계) 상단 버튼(`ActivityRecommendWizard.tsx`/`PlaceRecommendWizard.tsx`) |
| **다이얼로그 닫기**: 오버레이를 닫음(화면 이동 없음) | **텍스트 버튼 = "취소"** / **아이콘 전용 X = sr-only "닫기"** | 텍스트는 아이콘 없음, 아이콘 버튼은 `XIcon` | `<DialogClose>` | 결정 다이얼로그(`DeleteConfirmDialog` 등, §11) 하단 = "취소" 텍스트 버튼. 입력 폼 다이얼로그(`VisitedDialog` 등) 상단 X = `<span className="sr-only">닫기</span>`(`ui/dialog.tsx`) — 시각적으로는 안 보이고 스크린리더에서만 읽힌다 |

**규칙:**
- 같은 좌상단 위치·같은 `ArrowLeft` 아이콘이라도 **동작이 다르면 라벨도 달라야 한다.** 목적지 이동형과 상태 리셋형을 같은 라벨("홈으로" 등)로 뭉치지 않는다 — 과거 위저드 마법사 화면 상단 버튼이 모든 단계에서 `<Link href="/">홈으로</Link>`였던 것을 2단계 이상에서 "처음부터"(`reset()`)로 교체한 것이 이 판단 기준의 적용 사례다.
- **위저드 마법사 화면 상단 버튼은 단계(step)에 따라 동작 자체가 다르므로 라벨도 분기한다.** 1단계는 되돌릴 선택값이 없는 목적지 이동형("홈으로"), 2단계 이상은 이미 선택값이 쌓여 있어 초기화가 의미 있는 상태 리셋형("처음부터")이다. "위치가 같으니 라벨도 통일" 식으로 뭉치지 말 것 — 단계별로 실제 일어나는 동작이 다르면 라벨도 그 동작을 그대로 따라간다. 결과 화면(`showResult`) 상단·하단 버튼은 이 분기 대상이 아니며 항상 "처음부터"로 고정.
- `returnTo`처럼 목적지가 여러 개로 분기될 수 있는 목적지 이동형은, 그 목적지에 맞는 명사를 라벨에 그대로 반영한다(목록="목록으로", 추천 결과="추천 결과로") — 목적지와 무관하게 라벨을 고정하지 않는다.
- 다이얼로그 닫기는 §11의 "입력 폼 vs 결정 다이얼로그" 판단을 먼저 따르고, 그 결과로 남는 닫기 수단(텍스트 "취소" 또는 아이콘 X)에만 이 표의 텍스트를 적용한다. 두 수단을 한 다이얼로그에 동시에 노출하지 않는다(§11 그대로).
- "이전"(스텝 하단, 직전 단계로 이동)과 다이얼로그 안 개별 액션 버튼(예: DetailBlock 인라인 편집 "취소")은 이 표의 범위 밖 — 별도 규칙 없이 현행 유지.
