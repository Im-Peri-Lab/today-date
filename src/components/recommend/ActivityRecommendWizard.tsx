...
// push/replace의 의미 차이와 히스토리 전략은 wizardUrl.ts에 공통화 — 여기선 활동 전용
// 쿼리 문자열만 결합한다.
function pushActivityWizardState(s: ActivityWizardUrlState) {
  pushWizardUrl(buildActivityWizardQuery(s))
}
function replaceActivityWizardState(s: ActivityWizardUrlState) {
  replaceWizardUrl(buildActivityWizardQuery(s))
}
...
export function ActivityRecommendWizard() {
  ...
  useWizardUrlSync(readActivityWizardUrlState, (next, source) => {
