...
## 구현 완료 (누적, 기존 유지)
(기존 항목 전체 유지)
- **(신규) 활동 추천 위저드에 실내/실외 선택 단계가 추가됨** — 시간대 다음, 카테고리 앞. 필터는 area와 동일하게 선택적 (PR #83, 상세 → CHANGELOG 2026-07-27)
- **(신규) 다이닝 카드 정보 줄이 메모 없을 때 위치를 대신 보여줌** — 액티비티 카드와 동일한 규칙으로 대칭화 (PR #82, 상세 → CHANGELOG 2026-07-25)
- **(신규) 활동에 실내/실외 필수 속성(location_type)이 추가됨** — 폼·상세·리스트 필터·카드까지 전 화면 반영 (PR #81, 상세 → CHANGELOG 2026-07-25)
- **(신규) 카테고리 체계 개편 및 최상위 타입 표시 라벨 rename 완료** — 활동 카테고리 7종, 장소 카테고리 9종으로 재편, 표시 라벨 활동→액티비티/장소→다이닝(내부 용어는 유지) (PR #78~80, 상세 → CHANGELOG 2026-07-24 v2)
