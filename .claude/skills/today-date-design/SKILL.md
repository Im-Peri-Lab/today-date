...
- **추천 위저드**(`ActivityRecommendWizard.tsx`): Step 3(시간대 다음, 카테고리 앞)에 독립 단계로 배치 — 카테고리("무엇을 하는가")와 실내/실외("조건")는 서로 다른 축이라 한 화면에 섞지 않는다(카테고리 개편 때 확립한 "축 분리" 원칙과 일관, §10-J 폼 순서와는 별개로 위저드는 자체 단계 순서를 가짐: 소요시간→시간대→실내/실외→카테고리). UI는 `optionCard` 카드 2개(Step1/2와 동일하게 **선택 시 자동 다음 단계 진행**) + "상관없어요" 아웃라인 버튼(명시적 스킵, `LOCATION_TYPE_ICONS`/`LOCATION_TYPE_OPTIONS` 재사용). `duration==='overnight'` 시 시간대 단계를 건너뛰는 기존 로직은 유지되며 건너뛴 뒤 도착지가 실내/실외 단계(Step 3)로 바뀐다. **필터는 area와 동일하게 선택적**(`recommendActivities`의 `location_type` truthy 체크 — 미선택 시 필터 미적용) — 저장값(NOT NULL)과 무관한 별개 관심사라는 원칙은 `/list` 필터와 동일.
