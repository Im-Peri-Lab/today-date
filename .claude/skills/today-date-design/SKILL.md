...
- 크기 관례: 리스트 카드 메타 `h-3 w-3`, 상세 화면 DetailRow 메타 `h-3.5 w-3.5`, 헤더 내비게이션(`.headerNavBtn`) `h-6`, 카드 ⋮ 메뉴(`.iconBtn`) `h-5`.
